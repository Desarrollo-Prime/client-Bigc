...
const ROLES = {
    ...
    USER: 'Usuario'
        ...
        async function apiFetch(endpoint, method, body = null, requiresAuth = true, isFormData = false) {
            ...
            const options = {
                method,
                headers: isFormData ? {} : headers, // FormData maneja su propio Content-Type
        ...
        async function saveDocument() {
            ...
            const companyId = docCompany.value;
            const areaId = docArea.value || null; // Convertir a null si está vacío
            const name = docName.value;
            const description = docDescription.value;
            const statusId = docStatus.value;
            const file = docFile.files[0];

            if (!name || !companyId || !statusId || (!docId && !file)) {
            ...
            if (docId) { // Editar documento
                ...
                } else if (!file && currentDocumentFile) {
                    // Si no se seleccionó nuevo archivo y hay uno actual, no hacer nada especial
                    // El backend mantendrá el archivo existente si 'file' no se envía en PUT
                }
