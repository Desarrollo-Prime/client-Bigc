...
/* async function apiFetch(endpoint, method, body = null, requiresAuth = true, isFormData = false) {
    ...
    if (requiresAuth && authToken) {
        ...
        console.warn('[apiFetch] Attempted to make authenticated request but authToken is missing. User is not logged in or token expired.'); // Enhanced warning
    }

    // Para FormData, NO establecer Content-Type explícitamente.
    // El navegador lo hará automáticamente con el boundary.
    // Para otros tipos de cuerpo (JSON), sí establecer Content-Type.
    if (!isFormData) {
    ...
    const options = {
        method,
        // Si es FormData, los headers se construyen con el Authorization si existe,
        // pero sin el Content-Type. Si no es FormData, se usan los headers normales.
        headers: headers, // Ahora siempre se incluyen los headers, incluyendo Authorization
    ...
    if (response.ok) {
        // Algunas respuestas 204 (No Content) no tienen cuerpo
        ...
        // Mejorar el mensaje de error para 401
...
async function loadDashboardStats() {
    try {
        // Aunque no tienes endpoints específicos para "count", simulamos la carga
        // En una aplicación real, necesitarías endpoints como GET /users/count, /documents/count, /areas/count
        ...
        filteredDocs.forEach(doc => {
            ...
            row.innerHTML = `
                ...
                <td>
                    ...
                    <a href="${API_BASE_URL}/documents/${doc.id}/download" class="btn action-btn" style="background-color: #3498db; margin-left: 5px;" target="_blank">Descargar</a>
                </td>
            `;
...
async function showDocumentForm(docId = null) {
    ...
    // Asigna el nombre de usuario actual como autor (solo lectura)
    // Ya no hay un campo 'docAuthor', asumo que esto era un placeholder
    // document.getElementById('doc-author').value = currentUser.userName; 
    ...
    if (docId) {
        ...
        try {
            ...
            // Almacenar el archivo actual si se decide reemplazar
...
/* async function saveDocument() {
    ...
    // Asegurarse de que los valores se conviertan a números donde se espera, y se limpien de espacios para cadenas
    const companyId = parseInt(docCompany.value);
    // Convertir a null si está vacío o si parseInt resulta en NaN
    const areaId = docArea.value ? parseInt(docArea.value) : null; 
    const name = docName.value.trim();
    const description = docDescription.value.trim();
    const statusId = parseInt(docStatus.value);
    ...
    if (isNaN(companyId)) { // Verificar si parseInt resultó en NaN
        showAlert(docFormMessage, 'Debe seleccionar una "Compañía" válida.', 'danger');
        return;
    }
    if (isNaN(statusId)) { // Verificar si parseInt resultó en NaN
        showAlert(docFormMessage, 'Debe seleccionar un "Estado" válido.', 'danger');
        return;
    }
    if (!docId && !file) { // El archivo es obligatorio solo para nuevos documentos
        showAlert(docFormMessage, 'El campo "Archivo" es obligatorio para nuevos documentos.', 'danger');
        return;
    }

    const formData = new FormData();
    // Convertir los IDs numéricos a string antes de adjuntarlos a FormData.
    // Esto es crucial porque FormData siempre envía valores como cadenas.
    // La transformación de estas cadenas a números debe ocurrir en el backend.
    formData.append('companyId', companyId.toString()); 
    if (areaId !== null && !isNaN(areaId)) { // Solo adjuntar si areaId es un número válido y no nulo
    ...
    // REMOVIDO: userId no debe ser enviado desde el frontend. El backend lo obtiene del JWT.
    // La información del usuario que crea el documento se debe extraer del token de autenticación
    // en el backend para mayor seguridad y consistencia.

    // Log del contenido de FormData para depuración
    console.log("[saveDocument] FormData content:");
    for (let pair of formData.entries()) {
        console.log(pair[0] + ': ' + pair[1]);
    }
    console.log("[saveDocument] Current authToken:", authToken);
    console.log("Estos son los ids:", areaId );
    ...
    if (docId) { // Editar documento
        ...
        // Si no se selecciona un nuevo archivo y hay uno actual, el backend debería mantener el existente
        // si 'file' no se envía en la solicitud PUT. No es necesario adjuntar currentDocumentFile aquí.
    ...
    try {
        // El último 'true' indica que requiere autenticación y el penúltimo 'true' que es FormData
...
async function saveDocument() {
    ...
    // Asegúrate de que los valores se conviertan a números para las validaciones internas del frontend
    const companyId = parseInt(companyIdValue);
    // Si areaIdValue es una cadena vacía, areaId será null, de lo contrario, se parsea a número.
    ...
    // Adjuntamos los IDs como cadenas. FormData siempre los envía como cadenas.
    // La transformación de estas cadenas a números DEBE ocurrir en el backend.
    formData.append('companyId', companyId.toString());
    if (areaId !== null) {
        formData.append('areaId', areaId.toString());
    ...
    let method = 'POST';
    // Endpoint ajustado a '/documents' para coincidir con el controlador del backend
    let endpoint = '/documents'; 
    ...
    try {
        // Aquí apiFetch mostrará los IDs y el archivo en la consola
        const response = await apiFetch(endpoint, method, formData, true, true);
        showAlert(docMessage, `Documento ${docId ? 'actualizado' : 'subido'} exitosamente.`, 'success');
        showView(docManagementView, 'Gestión Documental');
        loadDocuments();
...
async function deleteDocument(docId) {
    if (confirm('¿Está seguro de que desea eliminar este documento? Esto también eliminará el archivo asociado.')) {
        try {
...
function replaceDocumentFile() {
    docFileGroup.style.display = 'block';
    docCurrentFileGroup.style.display = 'none';
    // No limpiar docFile.value aquí, ya que el usuario seleccionará uno nuevo
}
...
function validateEmail(email) {
    // Corrección aquí: Expresión regular más robusta para validar emails
    const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/; 
